...
/**
* terminal执行: node ./src/memory-test/memory/summarization-memory.mjs
* 
* 
* 总结策略
*/
...
async function summarizationMemoryDemo() {
  ...
  if (allMessages.length >= maxMessages) {
    ...
    // 2. 总结将被丢弃的旧消息
    const summary = await summarizeHistory(messagesToSummarize); // 总结后的summary存放在哪？？？下一个问题
...
async function summarizeHistory(messages) {
  ...
  /**
   * 
   * 1）直接用 JSON.stringify() 会输出大量元数据
   * 
   * 2）用 getBufferString 节省大量 token，且人类可读。
   * 
   */
  ...
  // console.log(messages, conversationText);
