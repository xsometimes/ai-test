...
/**
* terminal执行: node ./src/memory-test/memory/summarization-memory2.mjs
* 
* 
* 总结策略：根据 token 来触发总结，而不是消息条数
*/
...
async function summarizationMemoryDemo() {
  ...
  const enc = getEncoding("cl100k_base"); // 返回一个Tiktoken实例
  ...
  if (totalTokens >= maxTokens) {
    ...
    for (let i = allMessages.length - 1; i >= 0; i--) {
      ...
      if (recentTokens + msgTokens <= keepRecentTokens) {
        recentMessages.unshift(msg); // 在数组的开头添加一个或多个元素
    ...
    console.log("保留的消息:", recentMessages.map(m => {
      ...
      // encode(text, allowedSpecial, disallowedSpecial)将文本字符串编码成 token ID 数组
      // encode(text).length 计算 token 数量（成本估算）
