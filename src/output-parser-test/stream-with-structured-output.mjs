/**
 * terminal执行: node ./src/output-parser-test/stream-with-structured-output.mjs
 */
...
const schema = z.object({
    ...
    famous_works: z.array(z.string()).optional().describe("著名作品列表"),
    biography: z.string().optional().describe("简短传记")
});
...
const prompt = `详细介绍莫扎特的信息，json格式输出。`;

console.log("🌊 流式结构化输出演示（withStructuredOutput）\n");

try {

    const stream = await structuredModel.stream(prompt);  // invoke 改成stream
    let chunkCount = 0;
    let result = null;
