/**
 * terminal执行: node ./src/output-parser-test/test/mini-cursor.mjs
 */
...
/* 重要规则前两条：防御性设计。通过告诉 AI 规则，避免产生“在已切换目录后又执行 cd”的错误命令 */
    ...
    for (let i = 0; i < maxIterations; i++) {
        ...
        /* 用 Map 是因为：
        1）AI 可能同时调用多个工具（如同时写多个文件）
        2）每个工具需要独立记录“已打印了多少字符”
        3）用 ID 区分不同的工具调用，互不干扰
        这是处理多路并发流式输出的标准做法。 */
                ...
                for (const toolCall of parsedTools) {
                    if (toolCall.type === 'write_file' && toolCall.args?.content) {
                        ...
                        if (currentContent.length > previousLength) {
                            ...
                            process.stdout.write(newContent); // // 实时打印正在写入的文件内容，为了让用户看到
