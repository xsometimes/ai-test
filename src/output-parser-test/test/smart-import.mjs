/**
 * terminal执行: node ./src/output-parser-test/test/smart-import.mjs
 */
...
async function extractAndInsert(text) {
  ...
  try {
    ...
    const prompt = `请从以下文本中提取所有好友信息，文本中可能包含一个或多个人的信息。请将每个人的信息分别提取出来，返回一个 JSON 数组格式，不要有其他文字。
    ...
    const insertDataLen = insertResult.affectedRows;
    console.log(`✅ 成功批量插入 ${insertDataLen} 条数据`);
    console.log(`   插入的ID范围：${insertResult.insertId} - ${insertResult.insertId + insertDataLen - 1}`);

    return {
      count: insertDataLen,
      insertIds: Array.from({ length: insertDataLen }, (_, i) => insertResult.insertId + i), // 生成一个包含连续 ID 的数组，用于批量插入后获取所有新插入行的 ID
    };
  } catch (err) {
